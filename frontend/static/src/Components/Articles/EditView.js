...
function EditView(props) {
	...
	return (
		<section id="edit-view">
            <button id="edit-toggle" onClick={toggleEdit} >{edit ? "Edit article?" : "stop editing"}</button>
            {edit ?
                <div id="article-view">
                    {post.title ? (
                        <h1>{post.title}</h1>
                ...
                <form id="edit-view">
                    {post.title ? (
                        <input name="title" id="edit-title" type="text" spellCheck="true" value={post.title} onChange={handleInput} ></input>
                    ) : (
                        <h1 >Select an article</h1>
                    )}
                    {post.text ? (
                        <textarea name="text" id="edit-text" type="text" wrap="soft" spellCheck="true" value={post.text} onChange={handleInput} ></textarea>
                    ) : (
                        <p>Article text here</p>
                    )}
                    {post.author ? (
                    ...
                    <div className="choice">
                        <button id="update" onClick={updateDraft} type="submit" >Update draft</button>
                        <button id="submit" onClick={submitDraft} >Submit draft</button>
                        <button id="delete" onClick={deleteDraft} >Delete draft</button>
                    </div>
